...
const MyCampaigns = () => {
    ...
    const { data: myCampaigns, isPending, isLoading, error } = useQuery({
        queryKey: ['myCampaign'],
        queryFn: async () => await axiosPublic.get(`campaigns?email=${user?.email}`).then((res) => {
            return res.data
        })
    })
    ...
    const columns = [
        ...
        columnHelper.accessor("campDeadline", {
            cell: (info) => <span>{info.getValue()}</span>,
            header: "Deadline",
        }),
        columnHelper.accessor("raisedMoney", {
            cell: (info) => <span>$ {info.getValue()}</span>,
            header: "Raised Money",
        }),
